...
exports.verfiyResetPassword = asyncHandler(async (req, res, next) => {
...
exports. ResetPassword = asyncHandler(async (req, res, next) => {
  ...
  user.passwordResetVerified = undefined;
  await user.save();
  const token = await jwt.sign({userId:user._id, role:user.role }, process.env.SECRET_TOKRN, {expiresIn:'2h'})
  res.status(202).json({ message: "user created password... sucessfuly", token });
  next();
});
