...
exports.getLecuters = asyncHandler(async (req, res, next) => {
  const lecuters = await Lecuter.find({courseId});
  res.status(202).json({message:'list of lucters'})
})

exports.getLecuterById = asyncHandler(async (req, res, next) => {
  const lecuters = await Lecuter.findById(id);
  res.status(202).json({message:'list of lucters', lecuters})
})
