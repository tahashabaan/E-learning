...
exports.removeCart = asyncHandler(async (req, res, next) => {
 ...
 exports.applyDisCount = asyncHandler(async (req, res, next) => { 
  ...
  const disCount = await Discount.findOne({dcode:discount, expireDate:{4gt:Date.now()} });
  const cart = await Cart.findOne({discount});

  if(!disCount && disCount.expireDate)
    return next(new ErrorHandle({message:"the code not valid", statsCode:422}));

    cart.totlalPriceAfterDisCount = cart.totalPrice *disCount;
    await cart.save();
});
