...
exports.forgetPassword = asyncHandler(async (req, res, next) => {
  ...
  const hashedRandomNum = crypto
    ...
    .update(randomNum)
    .digest("hex");
  await User.findByIdAndUpdate(user._id, { passwordResetCode: hashedRandomNum });
...
const verfiyResetPassword = asyncHandler(async (req, res, next) => {
  ...
  if (!crypto.co(user.passwordResetCode == passwordResetCode))
    return next(
      new ErrorHandle({
  ...
  req.passwordResetCode = passwordResetCode;
  next();
});
