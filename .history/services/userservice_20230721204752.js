...
exports.updateLoggedME = asyncHandler( async(req, res, next)=>{
    const { name, email, slug,phone, profileImage } = req.body;
    const document= await doc.findByIdAndUpdate(req.userId, {
        name, email,slug,phone, profileImage 
    })
...
exports.changePasswordMe = asyncHandler( async(req, res, next)=>{
    const { password } = req.body;
    const saltPass = await bcrypt.genSalt(10);
    const hashPassword = await bcrypt.hash(password, saltPass)

     await doc.findByIdAndUpdate(req.userId, {
        password:hashPassword,
        passwordChangedAt:Date.now()
    })

    const token = await jwt.sign({userId:req.userId, role:req.role}, process.env.SECRET_TOKRN)
    
     res
     .status(204)
     .json({message:'updated password', token})
